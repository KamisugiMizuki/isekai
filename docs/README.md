...
面向普通用户的统一桌面入口、角色联络 / 辅助写作 / 跑团工作区，以及首次配置、错误定位与恢复。该设计已定稿 v1.0，尚未实施；不替代各应用层的业务规范。

- [界面设计目录](user-interface/README.md)
- [统一人机交互界面设计](user-interface/USER_INTERFACE_DESIGN.md)
- [首次使用、配置与错误恢复](user-interface/ONBOARDING_AND_RECOVERY.md)
...
1. 设计普通用户入口时先读 [用户界面定案](user-interface/README.md)；理解内核时先读 WorldRuntime 总纲、会话核心和叙事中介，确认世界真值、会话与表达边界。
