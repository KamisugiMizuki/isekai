...
| `isekai_core/runtime/` | 世界运行层：历法换算、时钟与倍率、时间线服务与水位推进、性格单元、生活线、认知接口、补卡与性格驱动入口、事件引擎（`events.py`）、角色自主提案（`planning.py`）、环境事实状态（`environment.py`） |
