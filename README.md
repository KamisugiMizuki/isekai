...
- **角色记忆**（MEMORY_SPEC，阶段 2–3 范围）：三层数据域（对话原文 / 经历 / 规范记忆）各守本分——角色说了某事不等于它发生过。提取走有界来源 + 单次调用 + 引用校验（来源必须真实、类型在闭集、一两句话），同源幂等、同事实去重、相反说法保留双方并建替代链；迟到提取按**来源时刻**结算强度、再按当前水位做冲突校验。召回先限定可访问集合（实例 + 线 + 角色 + 水位），全文与向量归一后融合排序，简报按预算打包并带来源与确信线索（只进生成上下文，不展示给用户）；只有实际被采纳的一轮才强化且同轮幂等，衰减按**世界时长**、冻结期间不衰减。embedding 走远程服务、不可用即退化全文召回；界面不提供记忆浏览 / 编辑入口。
