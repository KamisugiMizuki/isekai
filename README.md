...
- 日志分离：壳 `logs/shell.log`，核心 `logs/core.log`。
- 已验证行为：真实往返（输入 → UMP → 核心 → LLM → 渲染 + 投递回执）、关窗到托盘（核心继续跑）、硬杀壳不留孤儿核心、设置面读写（Key 打码）。
- 未做（阶段 1 起）：世界包 / 实例 / 角色卡创作、时间线与版本操作、安装包（`npm run tauri build`）。
