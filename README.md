...
> 状态：设计完成（总纲 + 9 篇 SPEC）；**阶段 0 已完成**——核心进程（UMP v1 / 最小会话核心 / 管理面）、开发用 CLI 客户端与 Tauri 桌面壳（sidecar 监督 / 内建聊天 / 设置 / 托盘）均已落地并实机验证；世界包、角色卡、世界实例、导入导出与生成器已随**阶段 1**落地（见「世界设定层」一节）；**阶段 2（世界运行层）已落地**——世界时钟与倍率、激活 / 冻结、离线补算、性格单元、生活线与最小认知接口（见「世界运行层」一节）；**阶段 3（事件引擎）已落地**——四族事件模板、确定性候选与每日预算、事实效果与失效方式、说法集合与获知链、角色经历与素材资格、创建期历史回填、角色自主提案与环境事实状态（见「事件引擎」一节）；**阶段 4（版本管理）已落地**——提交 / 分叉 / 回滚 / 自动提交 / 用户引入事件；**阶段 5（多角色披露）已落地**——默认隔离、显式授权、转述不冒充亲历、撤回只有回滚（见「多角色披露」一节）；——提交、分叉、回滚（覆盖语义）、自动提交；角色记忆（提取 / 隔离 / 冲突 / 全文召回 / 简报注入 / 衰减强化）已按 MEMORY_SPEC 阶段 2–3 范围落地，向量召回与索引生命周期随阶段 4；**应用层（OC 故事层）核心语义已落地**——输入分类闸、产品状态翻译、故事表达契约、用户可见面投影与版本 / 创作操作编排（见「OC 故事层」一节）、**应用层（Writing Assistant / 编剧层）核心语义已落地**——大纲约束与条目状态机、只读观察三层输出、候选生命周期、GM 直接变化经规则层联合提交、分支试演与回滚后重新评估（见「Writing Assistant（编剧层）」一节），OC 客户端界面与编剧层界面均属独立实现范围（**内核验收阶段未建壳**：三个应用层已有 `story.*` / `wa.*` / `trpg.client.*` 管理面；后续统一桌面设计已在 `docs/user-interface/` 定稿 v1.0；**U1（启动与联络）已实施**——首次设置向导（本机检查 → 连接 AI → 选择任务 → 准备材料 → 开始使用）、首页、角色联络、世界与素材入口、设置与「帮助与诊断」（Core Debugging 移入其下的高级调试）都进了既有桌面壳；**U2 的世界变化 / 时间线管理与单文件全量备份已实施**——世界详情里的时间线管理（启动 / 暂停、改名、归档、删除需键入名称）、「尝试世界变化」（已登记对象按名称 → 草案 → 确认后另开一条暂停的新线）、设置里的单文件全量备份（完整性清单、暂存校验、恢复前副本、切换与失败回退、自动轮换）；**U2 的创建向导（世界设定侧）也已实施**——六步向导（来源 → 世界设定 → 准备角色 → 检查与确认 → 创建世界 → 开始方式），分区目录 + 条目表单 + 锁定 + 校验定位 + 保存草稿 / 确认设定，不再需要手改 JSON 或命令行；**U2 的角色卡起草工作区与开发目录迁移也已实施**——角色卡在同一个向导里起草、逐字段改、字段级锁定并确认；首次设置与设置页都能把旧的开发目录整份搬进来（源目录保留、凭据不搬、迁移后全线暂停）；**U3（辅助写作）也已实施**——一个工作区四个分区（大纲 / 当前素材 / 推进建议 / 文字草稿）：大纲按六类条目组织并逐条记决定、按角色读当前素材并带来源标签、推进建议给候选卡（以此起草 / 另开分支试演 / 预览世界变化后确认应用）、文字草稿可保存与锁定并导出；**U4（跑团工作区）进行中**——核心侧已落码（规则插件独立登记簿、战役与场景显示名，见 `docs/user-interface/README.md` 实施进度）并随全量测试通过；界面工作区已落地但真壳探针尚未收口；**U5 部分实施**——发行件自带运行环境、数据根到 `%LOCALAPPDATA%\isekai`、组装脚本 `scripts/build_release.py` 与普通用户五步说明（安装程序与真人试用未做），未开放入口在界面里如实标注，详见该目录「实施进度」）。
> 本项目独立于 veranima-companion；后者作为设计参考与资产来源。

## 普通用户：五步开始（发行件）

不用命令行、不用写 JSON。拿到发行件（`isekai-0.1.0-win64.zip`）之后：

1. **解压**到任意一个你有写权限的目录（例如 `D:\isekai`），不要放在需要管理员权限的位置。
2. **双击 `isekai.exe`**：程序自带运行环境，不需要另外安装 Python 或别的软件。
3. **首次设置**：先「本机检查」（看目录读写与版本），再填 AI 服务的**地址、模型名与密钥**。密钥只存在你这台机器上，随时能在设置里改；没有 AI 也能先整理本机内容。
4. **准备材料**：在「世界与素材」里从**样例世界**开始（推荐），或者自己新建一份世界设定、起草角色卡。
5. **开始用**：想在聊天里认识角色，去「角色联络」；想整理故事走向，去「辅助写作」；想跑一局，去「跑团」。

出问题时界面会说清四件事：**操作对象、直接原因、已完成范围、下一步**；「帮助与诊断」里能看本机检查、日志位置与高级调试。
数据放在 `%LOCALAPPDATA%\isekai`（程序目录只放程序），设置 → 数据与备份里可以立即备份、恢复或搬迁。

> 发行件的组装：`python scripts/build_release.py`（壳 release 构建 + 自带解释器 + 核心源码 + 样例，产物在 `release/`）。
> 目前是 zip + 手动解压，安装程序尚未做；三条完整路径的真实试用记录也还没做（需要真人试跑）。

## 文档

- 设计文档地图：[`docs/README.md`](docs/README.md)
- 普通用户界面设计 v1.0（已定稿；U1–U3 已实施、U4 进行中、U5 部分实施——启动与联络、时间线管理、尝试世界变化、单文件全量备份、创建向导（世界设定 + 角色卡）、开发目录迁移、辅助写作工作区、跑团核心与规则登记、发行件组装与数据根，实施进度与已知边界见文末）：[`docs/user-interface/README.md`](docs/user-interface/README.md)
